...
class BertData():
    def __init__(self, args):
        self.args = args
        self.tokenizer = BertTokenizer.from_pretrained(r'C:\Users\Administrator\PycharmProjects\one\bertsum-chinese-LAI\bert-base-chinese-vocab.txt', do_lower_case=True)  ### change from 'bert-base-uncased' to 'bert-base-chinese'
        self.sep_vid = self.tokenizer.vocab['[SEP]']#所在位置信息
    ...
    def preprocess(self, src, tgt, oracle_ids):
        ...
        tgt_txt_ = [' '.join(sent) for sent in tgt]
        text_ = ' [SEP] [CLS] '.join(tgt_txt_)
        tgt_subtokens = self.tokenizer.tokenize(text_)
        #tgt_subtokens = tgt_subtokens[:510]
        tgt_subtokens = ['[CLS]'] + tgt_subtokens + ['[SEP]']
        ...
        return src_subtoken_idxs, tgt_subtoken_idxs, labels, segments_ids, cls_ids, src_txt, tgt_txt
...
def _format_to_bert(params):
    ...
    for d in jobs:
        ...
        if (b_data is None):
            continue
        indexed_tokens, tgt_subtoken_idxs, labels, segments_ids, cls_ids, src_txt, tgt_txt = b_data
        b_data_dict = {"src": indexed_tokens, "tgt": tgt_subtoken_idxs, "labels": labels, "segs": segments_ids, 'clss': cls_ids,
                       'src_txt': src_txt, "tgt_txt": tgt_txt}
        print('dd:', b_data_dict)
...
def _format_raw(raw_LCSTS_path, is_train=True):
    raw_LCSTS_file = open(raw_LCSTS_path, "r", encoding='utf-8')
    raw_LCSTS_str = raw_LCSTS_file.read()
    raw_LCSTS_str_list = raw_LCSTS_str.split("\n")
    
    num_line_el = 8 if is_train else 9#数据集中没事件文本split完八行or9行
    extract_line = [0, 2, 5] if is_train else [0, 3, 6]#0:id 2:summary 3:text
    num_el = len(raw_LCSTS_str_list)//num_line_el#一共多少文本事件
    
    json_list = []
    for i in range(num_el):
        doc = {"id": raw_LCSTS_str_list[i*num_line_el+extract_line[0]].strip(), #序列标号
        "tgt": raw_LCSTS_str_list[i*num_line_el+extract_line[1]].strip(), #summary
        "src": raw_LCSTS_str_list[i*num_line_el+extract_line[2]].strip()}#text文本

        json_list.append(doc)
    
    for i in json_list:
        num = re.findall(r'\d+', i['id'])#正则表达
        doc_id = int(num[0])
        i['id'] = doc_id
...
#C:\\Users\\Administrator\\AppData\\Roaming\\pyrouge\\settings.ini