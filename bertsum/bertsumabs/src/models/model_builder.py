...
class Bert(nn.Module):
    def __init__(self, large, temp_dir, finetune=False):
        ...
        if(large):
            ...
            self.model = BertModel.from_pretrained('bert-base-chinese', cache_dir=r'C:\Users\Administrator\PycharmProjects\one\bertsum-chinese-LAI\temp')

        self.finetune = finetune#ture
        #输入最多512个词（还要除掉[CLS]和[SEP]），最多两个句子合成一句。这之外的词和句子会没有对应的embedding
...
class ExtSummarizer(nn.Module):
    def __init__(self, args, device, checkpoint):#device:-1, None
        super(ExtSummarizer, self).__init__()
        self.args = args
        self.device = device
        self.bert = Bert(args.large, args.temp_dir, args.finetune_bert)#False, ../temp, True
        ...
        if (args.encoder == 'baseline'):
            ...
            self.ext_layer = Classifier(self.bert.model.config.hidden_size)#768
...
class AbsSummarizer(nn.Module):
    def __init__(self, args, device, checkpoint=None, bert_from_extractive=None):
        ...
        self.args = args
        self.device = device
        self.bert = Bert(args.large, args.temp_dir, args.finetune_bert)#false, ../temp, ture
