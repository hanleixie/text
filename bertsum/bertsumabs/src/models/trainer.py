...
class Trainer(object):
    ...
    def train(self, train_iter_fct, train_steps, valid_iter_fct=None, valid_steps=-1):
        ...
        step =  self.optims[0]._step + 1#1
        true_batchs = []
        accum = 0
        normalization = 0
        train_iter = train_iter_fct()

        total_stats = Statistics()#包含loss,n_corrert,c_docs,n_src_words,n_words,time
        report_stats = Statistics()
        self._start_report_manager(start_time=total_stats.start_time)

        while step <= train_steps:#train
            ...
            for i, batch in enumerate(train_iter):
                if self.n_gpu == 0 or (i % self.n_gpu == self.gpu_rank):
                    ...
                    num_tokens = batch.tgt[:, 1:].ne(self.loss.padding_idx).sum()#102,[6,18],将cls去掉为102个参数
    ...
    def _gradient_accumulation(self, true_batchs, normalization, total_stats,
                               report_stats):
        if self.grad_accum_count > 1:#jump->>
        ...
        for batch in true_batchs:
            ...
            outputs, scores = self.model(src, tgt, segs, clss, mask_src, mask_tgt, mask_cls)#[6, 17, 768],None
            batch_stats = self.loss.sharded_compute_loss(batch, outputs, self.args.generator_shard_size, normalization)

            batch_stats.n_docs = int(src.size(0))#6
