# -*- coding:utf-8 -*-
# @Time: 2021/2/1 11:00
# @File: sqlnet_condition_predict.py
# @Software: PyCharm
# @Author: xiehl
# -------------------------
import json
...
class SQLNetCondPredictor(nn.Module):
    ...
    def gen_gt_batch(self, split_tok_seq):
        ...
        if max_len < 1:
            max_len = 1
        ret_array = np.zeros((
            B, 4, max_len, self.max_tok_num), dtype=np.float32)#【16，4，1，200】
    ...
    def forward(self, x_emb_var, x_len, col_inp_var, col_name_len,
        ...
        num_col_att_val = self.cond_num_col_att(e_num_col).squeeze(-1)
        for idx, num in enumerate(col_num):
            if num < max(col_num):
                num_col_att_val[idx, num:] = -100
        num_col_att = self.softmax(num_col_att_val)
        K_num_col = (e_num_col * num_col_att.unsqueeze(2)).sum(1)#【16,100】
        ...
        num_att_val = self.cond_num_att(h_num_enc).squeeze(-1)
        ...
        if self.use_ca:
            ...
            col_att_val = self.cond_col_att(h_col_enc).squeeze(-1)
            for idx, num in enumerate(x_len):
                if num < max_x_len:
                    col_att_val[idx, num:] = -100
        ...
        cond_col_score = self.cond_col_out(self.cond_col_out_K(K_cond_col) +
                self.cond_col_out_col(e_cond_col)).squeeze(-1)

        # cond_col_score1 = self.cond_col_out(self.cond_col_out_K(K_cond_col) +
        ...
        if self.use_ca:
            op_att_val = torch.matmul(self.cond_op_att(h_op_enc).unsqueeze(1),
                    col_emb.unsqueeze(3))#.squeeze()#[16, 4, 56]为了单个样本注释掉.squeeze()
            for idx, num in enumerate(x_len):
                if num < max_x_len:
        ...
        if gt_where is not None:
            ...
            cond_str_score = self.cond_str_out(
                    ...
                    self.cond_str_out_col(col_ext)).squeeze(-1)
            for b, num in enumerate(x_len):
                if num < max_x_len:
                    cond_str_score[:, :, num:] = -100
        else:
            h_ext = h_str_enc.unsqueeze(1).unsqueeze(1)
            col_ext = col_emb.unsqueeze(2).unsqueeze(2)
            scores = []
