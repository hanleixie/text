...
class WordEmbedding(nn.Module):
    ...
    def gen_x_batch(self, q, col):
        ...
        if self.trainable:
            ...
            val_emb_array = np.zeros((B, max_len, self.N_word), dtype=np.float32)#将每个问题的词向量变成[batch-size，max_len,embed-size]
            for i in range(B):
                for t in range(len(val_embs[i])):
    ...
    def gen_col_batch(self, cols):
        ...
        return name_inp_var, name_len, col_len#列名向量化，长度，几个列
