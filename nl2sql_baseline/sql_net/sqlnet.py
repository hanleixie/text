...
# -------------------------
import sys
sys.path.append(r'C:\Users\Administrator\PycharmProjects\one\yonyou\nl2sql_baseline\sql_net\modules')
import torch
...
from aggregator_predict import AggPredictor
from selection_predict import SelPredictor
from sqlnet_condition_predict import SQLNetCondPredictor
from select_number import SelNumPredictor
from where_relation import WhereRelationPredictor
...
class SQLNet(nn.Module):
        ...
        for cur_q, ans in zip(q, gt_cond_seq):
            ...
            for idx, item in enumerate(record):
                ...
                if item[0]:
                    ...
                    temp_ret_seq.extend(list(range(temp_q.index(item[1])+1,temp_q.index(item[1])+len(item[1])+1)))#获取条件值的索引
    ...
    def forward(self, q, col, col_num, gt_where=None, gt_cond=None, reinforce=False, gt_sel=None, gt_sel_num=None):
        ...
        if self.trainable_emb:
            ...
            x_emb_var, x_len = self.embed_layer.gen_x_batch(q, col)#x_len：batch中每个问题的长度，[x_emb_var：batch_size，max_seq_len，word_embedding_size]
            col_inp_var, col_name_len, col_len = self.embed_layer.gen_col_batch(col)#列名向量化，长度，几个列
            sel_num_score = self.sel_num(x_emb_var, x_len, col_inp_var, col_name_len, col_len, col_num)#[16,4]对问题的编码经过lstm，linear，softmax之后乘以编码
            ...
            sel_score = self.sel_pred(x_emb_var, x_len, col_inp_var, col_name_len, col_len, col_num)#【16,19】
            ...
            agg_score = self.agg_pred(x_emb_var, x_len, col_inp_var, col_name_len, col_len, col_num, gt_sel=pr_sel, gt_sel_num=pr_sel_num)#【16,4,6】

            where_rela_score = self.where_rela_pred(x_emb_var, x_len, col_inp_var, col_name_len, col_len, col_num)#【16,3】

            cond_score = self.cond_pred(x_emb_var, x_len, col_inp_var, col_name_len, col_len, col_num, gt_where, gt_cond, reinforce=reinforce)#4=>[16,5]
    ...
    def loss(self, score, truth_num, gt_where):
        ...
        sel_num_truth = torch.from_numpy(np.array(sel_num_truth)).long()#.astype(float))
        if self.gpu:
